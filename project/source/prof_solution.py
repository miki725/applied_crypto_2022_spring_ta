...
def get_master_info(pw, salt):
    raw = hashlib.pbkdf2_hmac(hash_name  = 'sha256',
                              password   = pw,
                              salt       = salt,
...
class EncFileInfo:
    ...
    def add_wildcard_terms(self, arr):
        ...
        for item in arr:
            ret.add(item)
            for i in range(WC_SMALLEST_WD, len(item) + 1):
                ret.add(item[:i] + '*')
    ...
    def decrypt_setup(self, pw):
        try:
            ...
            self.terms     = [bytearray.fromhex(x) for x in
                              self.metadata['terms']]
        except:
    ...
    def validate_mac(self):
        ...
        if mac != self.mac:
            err(f"{self.name}: Invalid MAC; ciphertext "
...
def op_encrypt(file_list, print_keys):
    ...
    try:
       pw    = bytes(getpass.getpass(prompt='Password: '), encoding='utf8')
    except:
        for fobj in files:
...
def parse_argv_and_go():
    ...
    parser.add_argument('-e',
                        action = 'store_true',
                        help   = 'Encrypt file(s) [default]')
    parser.add_argument('-d',
                        action = 'store_true',
                        help   = 'Decrypt file(s)')
    parser.add_argument('-s',
                        action = 'store_true',
                        help   = 'Search files in the current directory')
    parser.add_argument('-j',
                        action = 'store_true',
                        help   ='Output the master keys associated with files.')
